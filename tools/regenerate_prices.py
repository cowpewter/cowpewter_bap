...
def emit_script(values):
    """Write the merged table as a KubeJS script.

    Tags ('#namespace:path') go in their own object: a lookup tries the exact
    item id first and only falls back to testing tag ingredients on a miss.
    """
    ...
    items = {k: v for k, v in sorted(values.items()) if not k.startswith("#")}
    tags = {k[1:]: v for k, v in sorted(values.items()) if k.startswith("#")}

    def block(name, table):
        lines = [f"global.{name} = {{"]
        lines += [f"  '{k}': {coins(v)}," for k, v in table.items()]
        lines.append("};")
        return "\n".join(lines)

    with open(SCRIPT_OUT, "w") as fh:
        fh.write("// GENERATED by tools/regenerate_prices.py -- do not edit\n")
        fh.write(f"// {len(items)} items, {len(tags)} tags, MULTIPLIER {MULTIPLIER}\n")
        fh.write("// Values are in dollars; 1 dollar = 1 copper coin.\n\n")
        fh.write(block("BAP_PRICES", items) + "\n\n")
        fh.write(block("BAP_PRICE_TAGS", tags) + "\n")
    print(f"wrote {os.path.normpath(SCRIPT_OUT)} "
          f"({len(items)} items, {len(tags)} tags)")
